...
% Normalise Z using the training-time z-score parameters.
if isfield(model, 'mu') && isfield(model, 'sigma')
    Z = (Z - model.mu) ./ model.sigma;
end
% Unified classifier dispatch: new .classifier field, legacy .svm / .knn.
if isfield(model, 'classifier')
    classifiers = model.classifier;
elseif isfield(model, 'svm')
    classifiers = model.svm;
elseif isfield(model, 'knn')
    classifiers = model.knn;
else
    error('ISA:PYTHIAtest:noClassifier', ...
        'model.pythia has no recognised classifier field (classifier / svm / knn).');
end
nalgos = length(classifiers);
Y      = Y(:,1:nalgos);
Ybin   = Ybin(:,1:nalgos);

out.Yhat    = false(size(Ybin));
out.Pr0hat  = zeros(size(Ybin));
out.cvcmat  = zeros(nalgos, 4);

for ii = 1:nalgos
    clf = classifiers{ii};
    if isstruct(clf)
        % Legacy LIBSVM struct — convert predictions to binary.
        Yin = double(Ybin(:,ii)) + 1;
        [aux, ~, out.Pr0hat(:,ii)] = svmpredict(Yin, Z, clf, '-q');
        out.Yhat(:,ii) = aux == 2;
    elseif isa(clf, 'ClassificationSVM') || isa(clf, 'ClassificationKNN')
        [out.Yhat(:,ii), aux] = clf.predict(Z);
        out.Pr0hat(:,ii) = aux(:,1);
    else
        fprintf('  -> No model found for algorithm %d; predictions set to NaN.\n', ii);
        out.Yhat(:,ii)   = NaN;
        out.Pr0hat(:,ii) = NaN;
    end
    out.Yhat(:,ii) = logical(out.Yhat(:,ii));
    aux = confusionmat(logical(Ybin(:,ii)), out.Yhat(:,ii));
    if numel(aux) == 4
        out.cvcmat(ii,:) = aux(:)';
    end
end
