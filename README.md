...
- ```opts.clust.K``` number of clusters which corresponds to the final number of features returned. The routine assumes at least 3 clusters and no more than the number of features. Ideally it **should not** be a value larger than 10.
- ```opts.clust.NTREES``` number of threes used by the Random Forest models. Usually, this setting does not need tuning.
- ```opts.clust.MaxIter``` number of iterations used to converge the k-means algorithm. Usually, this setting does not need tuning.
- ```opts.clust.Replicates``` number of repeats carried out of the k-means algorithm. Usually, this setting does not need tuning.
