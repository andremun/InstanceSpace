...
**3D CLOISTER boundary.** For a 3D projection, `CLOISTER` now computes a 3D convex hull and returns its triangulation in `model.cloist.ZedgeFaces`/`ZecorrFaces`. `obj.plot('boundary')` and `scriptpng`'s `distribution_boundary.png` draw it as a translucent surface. A 3D model built with an earlier version still raises `ISA:InstanceSpace:boundaryNot3D`, since its stored boundary ignores the third coordinate; rebuild the `cloister` stage to fix it ([#50](https://github.com/andremun/InstanceSpace/issues/50)).
