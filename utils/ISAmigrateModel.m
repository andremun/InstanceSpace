...
if ~isfield(model, 'pilot') || ~isfield(model.pilot, 'Z') || ...
    ...
    warning('ISA:ISAmigrateModel:cannotRetrainPythia', ...
        ['model.pythia holds LIBSVM-format classifiers, which have no predict() method ' ...
         ...
         '(model.pilot.Z and model.data.Y/Ybin/Ybest/algolabels) are missing. Skipping ' ...
         'retraining: field names are still renamed (e.g. pythia.svm -> pythia.classifiers), ' ...
         'but the classifiers themselves remain legacy LIBSVM structs, unusable without the ' ...
         'LIBSVM MEX-files. Re-run build() to retrain with the current registry.']);
    return;
end
pyOpts = struct();
